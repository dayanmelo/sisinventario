...
@section('content_header')
    <div class="container-fluid">
        <div class="row mb-2">
            <div class="col-sm-6">
                <h1><b>Cierres</b>/Modificar Cierre</h1>
            </div>
            <div class="col-sm-6">
    ...
    <div class="row">
        <div class="col-md-12">
            <div class="card card-outline card-warning">
                <div class="card-body">
                    <form action="{{url('/admin/cierres',$cierre->id)}}" id="form_cierre" method="post">
                        @csrf
                        @method('PUT')
                        <input type="hidden" name="cierre_id" id="cierre_id" value="{{$cierre->id}}">
                        <div class="row">
                                    ...
                                    <table class="table table-sm table-striped table-bordered" id="tmp">
                                        ...
                                        @foreach($cierre->detallescierre as $detalle)
                                            ...
                                            <tr>
                                                ...
                                                <td style="text-align: center;vertical-align: middle" class="cantidad">{{$detalle->cantidad}}</td>
                                                <td>
                                                    <input type="number" class="form-control" id="precio_compra" name="precio_compra[]" value="{{$detalle->precio_compra}}">
                                                </td>
                                                <td>
                                                    <input type="number" class="form-control" id="precio_venta" name="precio_venta[]" value="{{$detalle->precio_venta}}">
                                                </td>
                                                <td id="subtotal" class="precio-total" style="text-align: center; vertical-align: middle"></td>
                                                <td id="subganancia" class="ganancia" style="text-align: center; vertical-align: middle"></td>
                                ...
                                <div class="row">
                                    <div class="col-md-8">
                                        <div class="form-group">
                                            ...
                                            <input type="datetime-local" class="form-control" value="{{$cierre->fecha}}" id="fecha" name="fecha">
                                            @error('fecha')
                                            <small style="color: red">{{$message}}</small>
                                            @enderror
                                        </div>
                                ...
                                <div class="row">
                                    <div class="col-md-12">
                                        <div class="form-group">
                                            ...
                                            <input type="text" style="background-color: yellow; color: black;text-align: center;font-size: 20px" class="form-control" id="total" name="total" value="{{$cierre->precio_total}}" readonly>
                                            @error('total')
                                            <small style="color: red">{{$message}}</small>
                                            @enderror
                                        </div>
                                ...
                                <div class="row">
                                    <div class="col-md-12">
                                        <div class="form-group">
                                            ...
                                            <input type="text" style="background-color: yellow; color: black;text-align: center;font-size: 20px" class="form-control" id="ganancia" name="ganancia" value="{{$cierre->ganancia}}" readonly>
                                            @error('total')
                                            <small style="color: red">{{$message}}</small>
                                            @enderror
                                        </div>
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            $('#tmp tbody').on('keypress', 'input[name="precio_compra[]"], input[name="precio_venta[]"]', function(event) {
                if (event.key === 'Enter') {
                    ...
                    if (precioVenta === '' || parseFloat(precioVenta) <= 0) {
                        ...
                        actualizarPrecioEnBD(row);
                    }
                }
            });

            // Evento al cambiar el valor del precio de venta o compra
            $('#tmp tbody').on('change', 'input[name="precio_venta[]"], input[name="precio_compra[]"]', function() {
                var row = $(this).closest('tr');
                calcularSubtotal(row);
                guardarPreciosEnLocalStorage();
                recalcularTotalGeneral();
            ...
            function actualizarPrecioEnBD(row) {
                var precioVenta = parseFloat(row.find('input[name="precio_venta[]"]').val());
                var precioCompra = parseFloat(row.find('input[name="precio_compra[]"]').val());
                var cantidad = parseInt(row.find('.cantidad').text()); // Asume que la cantidad está en un <td> con clase .cantidad
                var detalleId = row.find('.delete-btn').data('id');

                // Calcular precio total y ganancia
                var precioTotal = precioVenta * cantidad;
                var ganancia = (precioVenta - precioCompra) * cantidad;

                // Actualizar en el DOM
                row.find('.precio-total').text(precioTotal);
                row.find('.ganancia').text(ganancia);

                // Enviar la actualización con AJAX
                $.ajax({
                    url: "{{url('admin/cierres/detalles')}}/" + detalleId,
                    method: 'PUT',
                    data: {
                    ...
                    success: function(response) {
                        console.log('Precios actualizados:', response.message);
                    },
                    error: function(xhr) {
        ...
        $('.delete-btn').click(function () {
            ...
            if (id){
                $.ajax({
                    url:"{{url('/admin/cierres/detalle')}}/"+id,
                    type:'POST',
                    data:{
                        _token:'{{csrf_token()}}',
                        _method:'DELETE',
        ...
        $('#codigo').on('keyup',function (e) {
            if (e.which === 13){
                ...
                var id_cierre = $('#cierre_id').val();

                if (codigo.length > 0){
                    $.ajax({
                        url:"{{route('admin.detalle.cierres.store')}}",
                        method:'POST',
                        data:{
                            _token:'{{csrf_token()}}',
                            codigo: codigo,
